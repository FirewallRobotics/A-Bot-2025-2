...
public class AlignWithNearest extends Command {
  ...
  private final DoubleLogEntry robotXLog;
  private final DoubleLogEntry robotYLog;
  private final DoubleLogEntry robotRotLog;
  ...
  public AlignWithNearest() {
    // Initialize DataLog entries for target and robot positions
    ...
    robotXLog = new DoubleLogEntry(DataLogManager.getLog(), "/alignment/robot/x");
    robotYLog = new DoubleLogEntry(DataLogManager.getLog(), "/alignment/robot/y");
    robotRotLog = new DoubleLogEntry(DataLogManager.getLog(), "/alignment/robot/rotation");
  }
  ...
  public Pose2d getReefLocationInFieldSpaceWithOffset(double yOffset, Pose3d reeflocation) {
    ...
    if (reefLocationpPose2d == null) {
      DataLogManager.log("No reef location found, initiating scan");
      ...
      // Log robot position
      robotXLog.append(RobotFieldSpace.getX());
      robotYLog.append(RobotFieldSpace.getY());
      robotRotLog.append(RobotFieldSpace.getRotation().getDegrees());
      ...
      DataLogManager.log(
          String.format(
              "Calculated reef position: X=%.2f, Y=%.2f, Rot=%.2f", xActual, yActual, rotActual));
  ...
  public void initialize() {
    ...
    DataLogManager.log("AlignWithNearest initialized. Available tags: " + tags.length);
    ...
    if (tags.length > 0) {
      ...
      if (TagLocation != null) {
        // Log target position
        targetXLog.append(TagLocation.getX());
        targetYLog.append(TagLocation.getY());
        targetRotLog.append(TagLocation.getRotation().getDegrees());

        DataLogManager.log(
            String.format(
                "Aligning to Tag %d at X=%.2f, Y=%.2f, Rot=%.2f",
                TagAligningToo,
                TagLocation.getX(),
                TagLocation.getY(),
      ...
      DataLogManager.log("No tags found, defaulting to zero drive command");
  ...
  public void execute() {
    ...
    if (tags.length > 0 && conditionalDriveCommand.isFinished()) {
      ...
      Pose2d TagLocation = VisionSubsystem.getTagPose2d(TagAligningToo);
      if (TagLocation != null) {
        // Log updated target position
        targetXLog.append(TagLocation.getX());
        targetYLog.append(TagLocation.getY());
        targetRotLog.append(TagLocation.getRotation().getDegrees());

        DataLogManager.log(String.format("Updating alignment to Tag %d", TagAligningToo));
  ...
  public boolean isFinished() {
    ...
    if (finished) {
      DataLogManager.log(
          String.format(
              "AlignWithNearest finished. Lost tag: %b, Canceled: %b",
              !VisionSubsystem.CanSeeTag(TagAligningToo),
              RobotContainer.driverXbox.back().getAsBoolean()
                  || RobotContainer.coralController.back().getAsBoolean()));
    }
