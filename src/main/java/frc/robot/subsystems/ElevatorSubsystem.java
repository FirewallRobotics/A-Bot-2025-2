...
public class ElevatorSubsystem extends SubsystemBase {
  ...
  public final Trigger atMin =
      new Trigger(
          () ->
              getLinearPosition()
                  .isNear(ElevatorSubsystemConstants.kMinElevatorHeight, Inches.of(3)));
  public final Trigger atMax =
      new Trigger(
          () ->
              getLinearPosition()
                  .isNear(ElevatorSubsystemConstants.kMaxElevatorHeight, Inches.of(3)));
  ...
  ElevatorFeedforward m_feedforward =
      new ElevatorFeedforward(
          ElevatorSubsystemConstants.kElevatorkS,
          ElevatorSubsystemConstants.kElevatorkG,
          ElevatorSubsystemConstants.kElevatorkV,
  ...
  private final Alert m_laserCanFailure =
      new Alert("LaserCAN failed to configure.", AlertType.kError);
  ...
  private final ProfiledPIDController m_controller =
      new ProfiledPIDController(
          ElevatorSubsystemConstants.kElevatorKp,
          ElevatorSubsystemConstants.kElevatorKi,
          ElevatorSubsystemConstants.kElevatorKd,
          new Constraints(
              ElevatorSubsystemConstants.kMaxVelocity,
              ElevatorSubsystemConstants.kMaxAcceleration));
  ...
  private final ElevatorSim m_elevatorSim =
      new ElevatorSim(
          ...
          ElevatorSubsystemConstants.kCarriageMass,
          ElevatorSubsystemConstants.kElevatorDrumRadius,
          ElevatorSubsystemConstants.kMinElevatorHeight.in(Meters),
          ElevatorSubsystemConstants.kMaxElevatorHeight.in(Meters),
          true,
                ...
                log.motor("elevator")
                    .voltage(
                        m_appliedVoltage.mut_replace(
                            m_motor.getAppliedOutput() * RobotController.getBatteryVoltage(),
                            Volts))
                    .linearPosition(
  ...
  public ElevatorSubsystem() {
    ...
    config
        .smartCurrentLimit(ElevatorSubsystemConstants.kElevatorCurrentLimit)
        .closedLoopRampRate(ElevatorSubsystemConstants.kElevatorRampRate)
        .closedLoop
  ...
  /**
   * Seed the elevator motor encoder with the sensed position from the LaserCAN which tells us the
  ...
  public void seedElevatorMotorPosition() {
    if (RobotBase.isSimulation()) {
      ...
      m_encoder.setPosition(
          Elevator.convertDistanceToRotations(
                  Millimeters.of(
        ...
        MathUtil.clamp(
            m_controller.calculate(getHeightMeters(), goal)
                + m_feedforward.calculateWithVelocities(
                    getVelocityMetersPerSecond(), m_controller.getSetpoint().velocity),
            -7,
            7);
  ...
  public double getHeightMeters() {
    return (m_encoder.getPosition() / ElevatorSubsystemConstants.kElevatorGearing)
        * (2 * Math.PI * ElevatorSubsystemConstants.kElevatorDrumRadius);
  }
  ...
  public double getVelocityMetersPerSecond() {
    return ((m_encoder.getVelocity() / 60) / ElevatorSubsystemConstants.kElevatorGearing)
        * (2 * Math.PI * ElevatorSubsystemConstants.kElevatorDrumRadius);
  }
