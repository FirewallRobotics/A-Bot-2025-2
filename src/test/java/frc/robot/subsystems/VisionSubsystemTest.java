...
class VisionSubsystemTest extends TestBase {
  ...
  void testCanSeeTag_WhenTagVisible() {
    ...
    try (MockedStatic<LimelightHelpers> mockedStatic = Mockito.mockStatic(LimelightHelpers.class)) {
      ...
      mockedStatic
          .when(() -> LimelightHelpers.getLatestResults(anyString()))
  ...
  void testDistanceToReef_WhenReefTagVisible() {
    try (MockedStatic<LimelightHelpers> mockedStatic = Mockito.mockStatic(LimelightHelpers.class)) {
      ...
      mockedStatic
          .when(() -> LimelightHelpers.setPipelineIndex(anyString(), anyInt()))
      ...
      mockedStatic
          .when(() -> LimelightHelpers.getRawFiducials(anyString()))
      ...
      mockedStatic.verify(
          () -> LimelightHelpers.setPipelineIndex(anyString(), eq(APRILTAG_PIPELINE_INDEX)));
    }
  }
  ...
  void testUpdatePositionOnField_WithValidTags() {
    ...
    try (MockedStatic<LimelightHelpers> mockedStatic = mockStatic(LimelightHelpers.class)) {
      ...
      mockedStatic
          .when(() -> LimelightHelpers.getBotPoseEstimate_wpiBlue_MegaTag2(anyString()))
  ...
  void testGetTagArea() {
    try (MockedStatic<LimelightHelpers> mockedStatic = Mockito.mockStatic(LimelightHelpers.class)) {
      ...
      LimelightHelpers.LimelightTarget_Fiducial mockTarget =
          mock(LimelightHelpers.LimelightTarget_Fiducial.class);
  ...
  void testCanSeeAlgae() {
    try (MockedStatic<LimelightHelpers> mockedStatic = Mockito.mockStatic(LimelightHelpers.class)) {
      mockedStatic
          .when(() -> LimelightHelpers.getTargetColor(anyString()))
